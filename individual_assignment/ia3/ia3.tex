...
algorithm to find a specific path of length 6 in the graph is $\frac{1}{6^6}$.

\vspace{2mm}\textit{Proof}: If the i$^{th}$ node of the path P is colored i, for all i, then the path will be found.
The probability for each node to be a specific color is $\frac{1}{6}$. The probability for 6 specific vertices to
each be a specific color is $\frac{1}{6^6}$. 
...
algorithm to find any path of length 6 in the graph is $3 * \frac{1}{6^6} - \frac{1}{6^7}$.

\vspace{2mm}\textit{Proof}: Combining the knowledge from Lemma 1 and Lemma 2 we get that the total probability
for the algorithm to find a path of length 6 is $3 * {1/6}^6$ but this doesn't account for the fact that
two of the paths are the same in all but one vertex. The two similar paths can therefore be correct at the same
time and the probability for this event to happen needs to be removed from the total probability.

For both paths to be correct, all 7 nodes would need to be a specific color, this means that the probability of
this event happening is $\frac{1}{6^7}$.

The total probability for the Color Coding algorithm to find any path of length 6 is therefore $3 * \frac{1}{6^6} - \frac{1}{6^7}$.
